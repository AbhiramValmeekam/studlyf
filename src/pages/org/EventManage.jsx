...
/**
 * Org EventManage — full event lifecycle management for a single event:
 * Registration → Participants → Teams → Submissions → Jury Evaluation →
 * Ranking → Results. Each tab shows the relevant data surface.
 *
 * Distinct from the legacy /organizer route — this lives under /org and is
 * gated to the Organisation role.
 */
  ...
  return (
    <>
      ...
      <PageTitle eyebrow={e.dates} title={e.name}>
        <Button to={`/org/evaluate/${id}`} size="sm" variant="flare">
          Open jury room ↗
        </Button>
      </PageTitle>
          ...
          <button
            ...
            className={`px-4 py-3 text-sm whitespace-nowrap border-b-2 transition-colors ${
              stage === s
                ...
                : 'border-transparent text-bone2 hover:text-bone'
            }`}
          >
            {s}
          </button>
      ...
      {stage === 'Teams'          && <Teams />}
      {stage === 'Submissions'    && <Submissions eventId={id} />}
      {stage === 'Jury Evaluation'&& <JuryOverview eventId={id} />}
...
function Registration({ e }) {
  return (
    <div className="grid sm:grid-cols-3 gap-4">
      <Card className="p-6">
        <p className="eyebrow text-bone2">Registered</p>
        <p className="font-display text-5xl text-flare mt-2">{e.registrations}</p>
      </Card>
      <Card className="p-6">
        <p className="eyebrow text-bone2">Capacity</p>
        <p className="font-display text-5xl text-bone mt-2">500</p>
      </Card>
      <Card className="p-6">
        <p className="eyebrow text-bone2">Fill rate</p>
        <p className="font-display text-5xl text-acid mt-2">
          {Math.round((e.registrations / 500) * 100)}%
        </p>
      </Card>
    </div>
  )
...
function Participants() {
  return (
    <Table
      head={['Name', 'College', 'Team', 'Status']}
      rows={eventDetail.participants.map((p) => (
...
function Teams() {
  return (
    <Table
      head={['Team', 'Members', 'Project', 'Status']}
      rows={eventDetail.teams.map((t) => (
...
function Submissions({ eventId }) {
  return (
    <>
      <div className="flex items-center justify-between mb-4">
        <p className="text-sm text-bone2">{eventDetail.submissions.length} submissions received</p>
        <Button to={`/org/evaluate/${eventId}`} size="sm" variant="flare">
          Open jury room ↗
        </Button>
      </div>
      <Table
        head={['Team', 'Repository', 'Demo', 'Score']}
        rows={eventDetail.submissions.map((s) => (
        ...
        <Card key={r.rank} className="p-5 flex items-center gap-4">
          <span
            className={`font-display text-3xl ${
              r.rank === 1 ? 'text-acid' : r.rank === 2 ? 'text-bone' : 'text-flare'
            }`}
          >
            #{r.rank}
          </span>
...
function Results() {
  ...
  return (
    <Card className="p-10 text-center">
      ...
      <p className="text-bone2 mt-2">
        Score {winner.score}/100 · results published to all participants.
      </p>
      <Button
        size="md"
        className="mt-6"
        onClick={() => alert('Prototype: results published + certificates issued.')}
      >
        Publish results ↗
      </Button>
    </Card>
